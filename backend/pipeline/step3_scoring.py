...
class ClipScorer:
    ...
    def _validate_llm_response(self, parsed_list: List[Dict], expected_count: int) -> tuple[bool, List[str]]:
        ...
        for i, result in enumerate(parsed_list):
            ...
            if 'final_score' not in result:
                missing_fields.append('final_score')
            if 'recommend_reason' not in result:
                missing_fields.append('recommend_reason')
            
            if missing_fields:
                errors.append(f"第 {i+1} 个结果缺少字段: {', '.join(missing_fields)}")
            ...
            if 'recommend_reason' in result:
                ...
                if not isinstance(reason, str) or len(reason) < 25 or len(reason) > 50:
                    errors.append(f"第 {i+1} 个结果的推荐理由长度不符合要求(25-50字): {len(reason)}字")
    ...
    async def _score_chunk_async(self, chunk_index: int, chunk_items: List[Dict]) -> List[Dict]:
        ...
        try:
            # 使用LLM进行批量评估
            scored_chunk_items = await self._get_llm_evaluation_async(chunk_items)

            if scored_chunk_items:
                logger.info(f"块 {chunk_index} 的LLM评估成功，返回 {len(scored_chunk_items)} 个结果")
            else:
                logger.warning(f"块 {chunk_index} 的LLM评估返回为空")

            return scored_chunk_items
            ...
            for clip in chunk_items:
                ...
                clip['recommend_reason'] = f"评分失败，使用默认分数: {str(e)[:50]}"
            return chunk_items
    ...
    def _score_chunk(self, chunk_index: int, chunk_items: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            if loop.is_running():
                ...
                return loop.run_until_complete(self._score_chunk_async(chunk_index, chunk_items))
        except RuntimeError:
        ...
        try:
            ...
            for i, original_clip in enumerate(clips):
                if i < len(parsed_list):
                    ...
                    original_clip['final_score'] = 0
                    original_clip['recommend_reason'] = "LLM未返回评分结果"

            return clips

        except Exception as e:
            logger.error(f"LLM批量评估失败: {e}")
            # 如果批量失败，为所有clips标记为默认分数（50分），而不是0
            # 这样至少有一些片段可以通过阈值筛选
            for clip in clips:
                clip['final_score'] = 50  # 使用中等分数而不是0
                clip['recommend_reason'] = f"批量评估失败，使用默认分数: {str(e)[:50]}"
            return clips

    async def _get_llm_evaluation_async(self, clips: List[Dict]) -> List[Dict]:
        """
        使用LLM进行批量评估，为每个clip添加 final_score 和 recommend_reason（异步版本，支持重试）
        """
        # 输入给LLM的数据不需要包含所有字段，只给必要的
        input_for_llm = [
            {
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt > 0 and last_errors:
                    ...
                    error_hint += "\n\n请严格按照要求重新输出，确保：\n1. 输出数量与输入数量完全一致\n2. 每个结果都包含 final_score 和 recommend_reason 字段\n3. 输出格式为有效的JSON数组"
                ...
                last_errors = [f"LLM调用异常: {str(e)[:100]}"]
        ...
        if parsed_list is None or not isinstance(parsed_list, list):
            ...
            for clip in clips:
                clip['final_score'] = 50
                clip['recommend_reason'] = "批量评估失败，使用默认分数"
            return clips
