...
class LLMConfig:
    ...
    max_retries: int = 3
    timeout_seconds: int = 120  # 优化：从30秒增加到120秒，减少超时率
...
class ProcessingParams:
    """处理参数"""
    chunk_size: int = 5000
    min_score_threshold: float = 70  # 100分制
...
class ProjectConfigManager:
    ...
    def get_llm_config(self) -> LLMConfig:
        ...
        # 使用LLMManager获取当前配置的API密钥
        from core.llm_manager import get_llm_manager
        llm_manager = get_llm_manager()
        
        # API密钥优先级：项目配置 > LLMManager配置 > 默认值
        api_key = llm_config.get("api_key")
        if not api_key:
            # 从LLMManager获取当前提供商的API密钥
            provider_type = llm_manager.settings.get("llm_provider", "dashscope")
            if provider_type == "dashscope":
                api_key = llm_manager.settings.get("dashscope_api_key", "")
            elif provider_type == "openai":
                api_key = llm_manager.settings.get("openai_api_key", "")
            elif provider_type == "gemini":
                api_key = llm_manager.settings.get("gemini_api_key", "")
            elif provider_type == "siliconflow":
                api_key = llm_manager.settings.get("siliconflow_api_key", "")
        
        if not api_key:
            raise ValueError(f"{provider_type} API密钥未在项目配置或设置中设置")
        
        # 获取模型名称
        model_name = llm_config.get("model_name", llm_manager.settings.get("model_name", "qwen-plus"))
