...
class ClipRepository(BaseRepository[Clip]):
    ...
    提供切片的数据访问操作，包括查询、创建、更新、删除、统计等功能
    """
    ...
    def get_high_score_clips(self, project_id: str, min_score: float = DEFAULT_MIN_SCORE, limit: int = DEFAULT_HIGH_SCORE_LIMIT) -> List[Clip]:
        """
        获取高分切片
    ...
    def bulk_create_clips(self, clips_data: List[Dict[str, Any]]) -> List[Clip]:
        """批量创建切片记录（优化性能）"""
        import uuid
        ...
        for clip_data in clips_data:
            ...
            clip = Clip(
                ...
                clip_metadata=clip_data.get("clip_metadata", {})
            )
            clips.append(clip)
        
        # 批量添加到会话
        self.db.bulk_save_objects(clips)
        self.db.commit()
        
        return clips
    
    def bulk_create_clips_simple(self, clips_data: List[Dict[str, Any]]) -> List[Clip]:
        """批量创建切片记录（简化版本，使用 bulk_insert_mappings）"""
        clips = []
