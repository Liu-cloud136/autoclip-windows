                ...
                {aiData.outline.topics.map((topic: any, index: number) => (
                  <li key={index} style={{ marginBottom: '8px' }}>
                    ...
                    {topic.subtopics && topic.subtopics.length > 0 && (
                      <Paragraph style={{ margin: '5px 0 0 20px' }}>
          ...
          {aiData.timeline.segments ? (
            <div>
              {aiData.timeline.segments.map((segment: any, index: number) => (
                <div key={index} style={{ marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px dashed #f0f0f0' }}>
                  ...
                  {segment.content && segment.content.length > 0 && (
                    <Paragraph style={{ margin: '5px 0 0 20px' }}>
                      <Text italic>内容摘要: {Array.isArray(segment.content) ? segment.content.slice(0, 3).join('；') : segment.content}</Text>
                    </Paragraph>
                  )}
                </div>
              ))}
            ...
            <div>
              ...
              <ul style={{ marginTop: '10px' }}>
                {aiData.clips.map((clip: any, index: number) => (
                  <li key={clip.id} style={{ marginBottom: '8px' }}>
          ...
          {aiData.rawOutputs ? (
            <div>
              {Object.entries(aiData.rawOutputs).map(([key, value]) => (
                <div key={key} style={{ marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px dashed #f0f0f0' }}>
                  <Text strong>{key}:</Text>
                  <Paragraph style={{ margin: '5px 0 0 20px', whiteSpace: 'pre-wrap' }}>
